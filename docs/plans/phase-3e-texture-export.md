...
2. **3e-2: `FTexturePlatformData` header parser (no mip bytes yet).** **The trickiest wire bytes — split into two reviewable PRs:**
   ...
   - **3e-2b:** `OptData` (bit 30: `ExtData` + `NumMipsInTail`), `CPUCopy` (bit 29, `FSharedImage` — read + cap `RawDataLen` + payload-bounded skip; not stored), `FirstMipToSerialize`, mip-count prefix. Adds `MAX_MIP_COUNT`, `MAX_MIPS_IN_TAIL`, `MAX_CPU_COPY_RAW_DATA_LEN` + their error variants. (`OptData`/`CPUCopy` are gated purely on the `PackedData` bits — no version check; the "5.4+" for CPUCopy is just when writers set bit 29.) The per-mip `FTexture2DMipMap` records move to 3e-3 (coupled with their resolution).
