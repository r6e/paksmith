...
- **Deferred (non-blocking, all reviewers concurred), documented in the PR body:** owner-row highlight (mitigated by the adjacent band), auto-scroll-into-view when the strip opens below the fold, a keyboard equivalent for Copy Path, and outside-click dismissal (iced 0.14 exposes no low-cost global click hook). Optional polish noted by UI/UX: per-button hover affordance and a small top margin on the band.
