...
| 2 | `strings` | variable | LE | `FString[]` | Per-entry: standard FString encoding (4-byte `i32` length + char bytes + null terminator; positive = UTF-8, negative = UTF-16 LE) — see [`../primitive/fstring.md`](../primitive/fstring.md). Each entry is referenced by the `Name` u32 indices in `DirectoryEntries` and `FileEntries`. |
...
- **Cap `DirectoryIndexSize`** against `MAX_IOSTORE_DIRECTORY_INDEX_BYTES` before allocating the read buffer. A `u32::MAX` `DirectoryIndexSize` would otherwise drive a 4 GiB read. The constant has a provisional **256 MiB** value in `docs/security/allocation-caps.md` §*Caps under audit* (borrowed by analogy from `MAX_FDI_BYTES` — both are "metadata sub-region read in full before parsing inside a larger container"); the Phase 8 IoStore-reader implementation PR MUST land the actual constant and replace the provisional value with an empirically-justified one if real-world cooked containers exceed 256 MiB. The cap MUST bound BOTH the on-disk buffer length AND the post-decryption interpretation.
...
- **Do not rely on "successful parse" as key-validation when the key source is untrusted.** AES-ECB provides no authentication; an adversary controlling both the container AND the key can craft a parseable buffer with any chosen key. See §*Variants — Encrypted containers* for the threat-model scope and the implication: trust in the key source is a prerequisite to parsing, not something the parser can validate from the container itself.
