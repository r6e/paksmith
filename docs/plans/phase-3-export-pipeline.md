...
- **Wire-stable `Display` for new `AssetParseFault` variants.** Phase 3 adds at least `UnsupportedPixelFormat`, `BulkDataOffsetOob`, `MipCountExceeded`, `VertexCountExceeded`. (Proprietary audio codecs route through `RawSoundHandler` raw passthrough rather than a fault variant — see §Scope.) Each gets a Display pin-table test per Phase 2g Task 1's pattern. `AssetParseFault::Display` is hand-rolled per repo convention.
