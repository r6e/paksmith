...
impl AudioOutput {
    ...
    /// Pauses playback.
    ///
    /// The Phase 7d transport resumes by re-feeding a fresh `SamplesBuffer` from
    /// the paused position (via [`AudioOutput::play_samples`]) rather than
    /// un-pausing in place, so there is no separate resume entry point.
