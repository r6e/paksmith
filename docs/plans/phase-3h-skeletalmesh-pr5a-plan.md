...
- [ ] **Step 1: Failing test.** `skin_weights_new_fixed_stride`: ctx with `FAnimObjectVersion >= UnlimitedBoneInfluences` + `>= IncreaseBoneIndexLimitPerChunk`. Bytes per the NEW metadata: `FStripDataFlags` + `bVariableBonesPerVertex=0(bool32)` + `maxBoneInfluences=4(u32)` + `numBones=...(u32)` + `numVertices=2(u32)` + `bUse16BitBoneIndex=0(bool32)` + `newData` bulk (`elementSize=1`, `elementCount = 2 verts × stride`) of raw influence bytes + lookup block (`FStripDataFlags` + `numLookupVertices=0(i32)`). For fixed-stride `!bVariableBonesPerVertex`, stride = `maxBoneInfluences × (boneIdxSize + weightSize)`; decode each vertex's bones/weights from `newData`. Assert `bone_indices`/`bone_weights`. RED.
- [ ] **Step 2: Run** → FAIL.
- [ ] **Step 3: Implement** the new branch per Task-1's pinned decode: read meta (`bVariableBonesPerVertex` bool32, `maxBoneInfluences`/`numBones`/`numVertices` u32 capped, `bUse16BitBoneIndex` bool32 if `>= IncreaseBoneIndexLimitPerChunk`, `bUse16BitBoneWeight` bool32 if `FUE5MainStream >= IncreasedSkinWeightPrecision`). If `!AVStripped`: `newData = read_bulk_array_header + bytes`; lookup block (`FStripDataFlags` + `numLookupVertices` i32 + `if !AVStripped: LookupData = read_bulk_array_header + count×u32`). Decode per-vertex from `newData`: fixed stride when `!bVariableBonesPerVertex`. **If Task 1 flagged variable-bones as complex: PR5a handles `!bVariableBonesPerVertex` (fixed stride) only; `bVariableBonesPerVertex == true` → leave bone data empty + a documented limitation (a fault or a `tracing::warn`), per Task 1's decision.** boneIdxSize = `bUse16BitBoneIndex ? 2 : 1`; weightSize = `bUse16BitBoneWeight ? 2 : 1` (UE4: both 1).
...
- [ ] **Step 3: Implement** `fn read_streamed_data<R: Read + ?Sized>(r, ctx, asset_path, b_has_vertex_colors: bool, sections: &[SkelMeshSection], lod: &mut SkeletalMeshLod) -> crate::Result<()>`:
  ...
  - adjacency: `if version_for(UE5_RELEASE_STREAM_GUID).is_none_or(|v| v < ADD_CLOTH_MAPPING_LOD_BIAS? NO — RemovingTessellation)` → use the Task-1 `RemovingTessellation` position; `&& !is_class_data_stripped(class, STRIP_FLAG_ADJACENCY_DATA)` → `let _ = read_multisize_index_container(...)`.
  - cloth: `if sections.iter().any(|s| s.correspond_cloth_asset_index >= 0) { skip the cloth buffer per Task 1 }`.
  - `FSkinWeightProfilesData`: read i32 count; count==0 → done; count>0 → per Task-1 (parse or `UnsupportedFeature`).
  - ray-tracing tail: `if has_ray_tracing(ctx) { skip SkipFixedArray(1) }` (Task-1 gate).
