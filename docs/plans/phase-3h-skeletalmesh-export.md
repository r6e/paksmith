...
- **`FSkelMeshSection` per-section record** (significantly differs from `FStaticMeshSection`; version gates are CUSTOM-VERSION-keyed, not UE-version-keyed).

  > **Note (PR3 correction):** paksmith's PR3 implements the **cooked `SerializeRenderItem` path** (18-field render layout), documented in [`skeletal-mesh.md` — cooked render path](../formats/mesh/skeletal-mesh.md). The 25-field list below is the **editor `FSkeletalMeshLODModel` constructor** path and is preserved as a reference; it is NOT reached by paksmith's cooked-asset target.

  The field order below is from the editor constructor; the TDD plan MUST read fields in this sequence:
  ...
  15. `bone_map: Vec<u16>` (counted `ushort[]`). LOD-local-to-global bone index translation. Counted-prefix is `i32`: (a) sign-check ≥ 0, (b) cap-check `<= MAX_BONE_MAP_ENTRIES_PER_SECTION` BEFORE `Vec::with_capacity` (per the master cap table — fires `BoundsExceeded { field: SkelSectionBoneMapCount, .. }`). Validate every entry post-read: `bone_map[i] < skeleton.bones.len()` (fires `BoneMapOob`, per H12 above).
