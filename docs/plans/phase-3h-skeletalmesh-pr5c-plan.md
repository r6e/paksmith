...
**PR-series:** PR1-4 + PR5a + PR5b merged. **PR5c (this) = the non-inlined/bulk LOD path — the LAST parser PR.** PR7 = `GltfSkeletalMeshHandler` (the glTF export + the bone-map remap-at-emit).
...
- [ ] **Post-merge** — remove worktree + branch, sync main; PR7 (the glTF exporter — the payoff) gets a fresh worktree + writing-plans pass. **The skeletal-mesh PARSER is complete after PR5c.**
...
- Deferred (documented): bulk-LOD geometry stays empty (external `.ubulk`); inline-payload bulk geometry parse needs a `read_from` payload-capture change → a future enhancement. The bone-map remap + the glTF exporter → PR7. **PR5c completes the skeletal-mesh parser.**
