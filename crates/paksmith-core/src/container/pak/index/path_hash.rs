...
impl PakIndex {
    ...
    pub(super) fn read_v10_plus_from<R: Read + Seek>(
        ...
        for _ in 0..dir_count {
            ...
            // Verified against `tests/fixtures/real_v10_minimal.pak` (and
            // siblings) generated by `trumank/repak`. The `unwrap_or`
            // correctly handles both forms — joined virtual path is
            // `(dir_name minus optional leading slash) + file_name`.
            ...
            for _ in 0..dir_file_count {
                ...
                let full_path = {
                    ...
                    crate::seams::seam_check!(
                        reserve_res,
                        crate::testing::oom::SeamSite::Pak(PakSeam::FdiFullPath)
                    );
