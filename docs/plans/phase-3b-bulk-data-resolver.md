...
11. **`BULKDATA_DuplicateNonOptionalPayload` reads-and-discards the extra fields.** The duplicate payload at the alternate offset is a redundancy mechanism; we read the primary record's payload and skip the duplicate's flags+size+offset. The skip happens during `FByteBulkData::read_from`, not at resolve time. Total skip is **16 bytes (no Size64Bit) or 20 bytes (with Size64Bit)** — composed of `DuplicateFlags: u32` (4 bytes) + `DuplicateSizeOnDisk` (4 or 8 bytes gated on Size64Bit) + `DuplicateOffset: i64` (always 8 bytes per paksmith's UE 4.4+ floor, which always implies `BULKDATA_AT_LARGE_OFFSETS`). The format doc's broader "12 / 16 / 16 / 20" matrix covers pre-4.3 content that paksmith doesn't accept.
