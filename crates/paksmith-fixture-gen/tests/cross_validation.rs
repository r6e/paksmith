...
//!
//! A third layer — the SHA1 byte anchor on one `_minimal` fixture per
//! version family — lives in `crates/paksmith-core/tests/fixture_anchor.rs`
//! so it runs in default `cargo test` without pulling repak.
//!
//! Closes part of #14.
...
// v4/v5 (issue #637): uncompressed/unencrypted fixtures pin paksmith's
// version dispatch for the two legacy versions between v3 and v6. v4
// (IndexEncryption) and v5 (RelativeChunkOffsets) share v3's flat-entry
// layout when neither encryption nor compression is used.
#[test]
fn paksmith_reads_repak_v4_minimal() {
    let reader = PakReader::open(fixture_path("real_v4_minimal.pak")).unwrap();
    assert_eq!(reader.version(), PakVersion::IndexEncryption);
    assert_eq!(reader.mount_point(), "../../../");
    let entries: Vec<_> = reader.entries().collect();
    assert_eq!(entries.len(), 1);
    assert_eq!(entries[0].path(), "Content/Example.uasset");
...
#[test]
fn paksmith_reads_repak_v5_minimal() {
    let reader = PakReader::open(fixture_path("real_v5_minimal.pak")).unwrap();
    assert_eq!(reader.version(), PakVersion::RelativeChunkOffsets);
    assert_eq!(reader.mount_point(), "../../../");
    let entries: Vec<_> = reader.entries().collect();
    assert_eq!(entries.len(), 1);
    assert_eq!(entries[0].path(), "Content/Example.uasset");
...
// v4/v5 (issue #637) cross-parser agreement.
#[test]
fn cross_parser_agreement_v4_minimal() {
    assert_cross_parser_agreement("real_v4_minimal.pak");
}

#[test]
fn cross_parser_agreement_v4_multi() {
    assert_cross_parser_agreement("real_v4_multi.pak");
}

#[test]
fn cross_parser_agreement_v4_mixed_paths() {
    assert_cross_parser_agreement("real_v4_mixed_paths.pak");
}

#[test]
fn cross_parser_agreement_v5_minimal() {
    assert_cross_parser_agreement("real_v5_minimal.pak");
}

#[test]
fn cross_parser_agreement_v5_multi() {
    assert_cross_parser_agreement("real_v5_multi.pak");
}

#[test]
fn cross_parser_agreement_v5_mixed_paths() {
    assert_cross_parser_agreement("real_v5_mixed_paths.pak");
}
