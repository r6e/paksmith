...
// `validate_unversioned_usmap_parser_parity` removed alongside #352/
// #353/#356: the `unreal_asset@f4df5d8` oracle's usmap reader at
// `unreal_asset_base/src/unversioned/mod.rs` shares the exact bugs
// paksmith was fixing:
//   - `ASSET_MAGIC = u16::from_be_bytes([0xc4, 0x30])` = 0xC430 (byte-
//     inverted vs CUE4Parse's `FileMagic = 0x30C4`).
//   - `name_length as usize - 1` for name reads (the same off-by-one).
// They agreed because both were broken in the same direction. The
// replacement is `crates/paksmith-fixture-gen/src/external_usmap.rs`,
// which emits CUE4Parse-spec-correct byte fixtures and pins them via
// the integration test `crates/paksmith-core/tests/
// external_usmap_integration.rs`. Asset-level decode coverage is
// unchanged (still pinned by `paksmith_core::testing::usmap::tests`).
