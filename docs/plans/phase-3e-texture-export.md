...
> **⚠️ FOLLOW-UP HAZARD — version floor is looser than intended (`fix(asset):` next, per user decision).** The 3e-3 segment-2-entry R1 architect surfaced, and raw-source verification confirmed, that paksmith's **enforced** UE4 floor (`summary.rs`: `file_version_ue4 >= VER_UE4_NAME_HASHES_SERIALIZED = 504`) is looser than its **documented intent** (UE 4.21–4.27). CUE4Parse's verbatim `EGame`→`FileVersionUE4` arms map `504 = GAME_UE4_12` (NOT UE4.21 — the `version.rs` "UE4.21 = 503" comment is wrong), `GAME_UE4_20 → 516`, `GAME_UE4_21 → 517`; and `legacy_file_version == -7` is a broad format-generation marker ("texture allocation info removed", per `FPackageFileSummary.cs`), not a 4.21 gate. So paksmith currently **admits UE4.12–4.19 cooked assets**, for which the texture parser's `skipOffset` (read `i64`; `i32` on the wire below `GAME_UE4_20`) **and the already-merged per-mip `SizeZ` gate** (3e-2/3e-3a, same `>= GAME_UE4_20` boundary, read unconditionally) read the wrong layout → 4-byte desync → caught by the dimension caps → degrade to `Generic` (fail-loud, not silent corruption). **Fix:** raise the summary-parser floor to object version **517** (`= GAME_UE4_21`), matching the documented intent and validating the `>= GAME_UE4_20` assumption that SizeZ + skipOffset (and likely other gates) already rely on. Note the inherent 4.19/4.20 object-version collision (both `→ 516`) means no object-version proxy is exact at that boundary; `517` cleanly excludes 4.20-and-below per intent.
