...
**PR-series (now 7):** PR1-4 + off-by-one merged. **PR5 sub-split → PR5a (this) = single inlined-LOD blob; PR5b = multi-LOD iteration + post-loop tail + non-inlined + bone-map remap.** PR7 = `GltfSkeletalMeshHandler`. (Tell the user.)
...
- Deferred to PR5b: multi-LOD iteration, the post-loop tail (dummyObjs + numInlinedLODs/numNonOptionalLODs + UV-channel skip), non-inlined FByteBulkData, bone-map LOD-local→global remap. To PR7: the glTF exporter. Limitations (Task-1-dependent): variable-bones decode, non-empty skin-weight profiles, UE5 u16 weights into `[u8;8]` — documented.
