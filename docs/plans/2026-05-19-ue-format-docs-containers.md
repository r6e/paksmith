...
- Working in a worktree under `.claude/worktrees/docs+ue-format-docs-containers/` (per the worktree-first rule — never commit feature work in the primary checkout).
...
Expected: ≥ 1. If 0, stop and resume PR 1 first.

- [ ] **Step 2: Create the worktree from origin/main**

From the primary checkout root:

Run: `git worktree add .claude/worktrees/docs+ue-format-docs-containers -b docs/ue-format-docs-containers origin/main`
Expected: `Preparing worktree (new branch 'docs/ue-format-docs-containers')` followed by a HEAD-at-origin/main message.

- [ ] **Step 3: Switch session cwd into the worktree**

Run: `cd .claude/worktrees/docs+ue-format-docs-containers && pwd && git branch --show-current`
Expected: prints the worktree path and `docs/ue-format-docs-containers`.

All subsequent commands in this plan run with the worktree as cwd. Do NOT use `git -C` to reach back to the primary or to other worktrees.

- [ ] **Step 4: Verify the framework scaffold is present**

Run: `ls docs/formats/container/README.md docs/formats/TEMPLATE.md docs/formats/CONVENTIONS.md docs/formats/README.md`
...
Run: `git ls-remote https://github.com/trumank/repak HEAD | cut -f1` — note as `<REPAK_SHA>`.
Run: `git ls-remote https://github.com/FabianFG/CUE4Parse HEAD | cut -f1` — note as `<CUE4PARSE_SHA>`.

If a repo URL has moved, find the current canonical home via web search.

- [ ] **Step 3: Verify hex-anchor offset against a real fixture**
...
- [ ] **Step 1: Capture this branch's HEAD SHA**

Run: `git rev-parse --short HEAD`
Note as `<SHA>` — goes in the `Last verified` column for `pak.md` only. The three IoStore stubs use `n/a`.

- [ ] **Step 2: Look up oracle SHAs one final time**
...
Use the same SHAs the per-doc commits cited, so the inventory and the doc
references agree. If different SHAs were captured during per-doc
authoring, prefer the inventory matching what shipped in the docs.

- [ ] **Step 3: Add four rows to the inventory table**

Locate the inventory table in `docs/formats/README.md` (the line beginning
`| Doc | Doc status | Parser status |`).

Use the Edit tool to insert four new rows immediately after the separator
row (or after the primitive rows, if PR 2 has already landed and added
them — verify with `grep -n "^|" docs/formats/README.md`).

The rows to insert. The IoStore rows use `partial` (not `stub`) because every H2 section is filled with prose, including explicit TODO markers in Caps & limits and Verification — that matches the spec's `partial` definition ("some sections complete, others marked `TODO`") rather than its `stub` definition ("1–2 paragraphs + references"):
...
Use Edit with:
- `old_string`: the line immediately above where the new rows belong (the
  separator row, or the last existing row if PR 2 has landed). Verify with
  `grep -n "^|" docs/formats/README.md` first.
- `new_string`: that line + `\n` + the four rows.

- [ ] **Step 4: Run the status-enum linter**

Run: `cargo run -p paksmith-doc-lint --release -- status-enum docs/formats/README.md`
Expected: exits 0. The three `partial | not impl` IoStore rows are clean
(no smell-warning combinations — `partial` doesn't trip the
`complete | not impl` warn or the `stub | complete` warn). The
`complete | complete` pak row is also clean.

- [ ] **Step 5: Run the required-headings linter against all docs**

Run: `cargo run -p paksmith-doc-lint --release -- required-headings docs/formats/`
Expected: exits 0.

- [ ] **Step 6: Verify the file tree matches the inventory**

Run: `ls docs/formats/container/*.md | sort`
Expected:
```
docs/formats/container/README.md
docs/formats/container/iostore-ucas.md
docs/formats/container/iostore-uptnl.md
docs/formats/container/iostore-utoc.md
docs/formats/container/pak.md
```

Run: `grep -c "container/" docs/formats/README.md`
Expected: ≥ 4 (one per row; the family heading in the front-matter list also matches the pattern so this may show more).

- [ ] **Step 7: Run typos against the new docs**

Run: `typos docs/formats/container/` (install with `cargo install typos-cli` if needed, version pinned to the CI workflow's SHA at `5374cbf686e897b15713110e233094e2874de7ef`).
Expected: no false positives. UE/UE4/UE5 are already in the allowlist; domain terms like `IoStore`, `FNV`, `Oodle`, `Zlib` may flag — add to `_typos.toml` only after confirming the prose can't be reworded.

- [ ] **Step 8: Run `cargo doc -D warnings` to pre-empt CI**
...
One complete-complete row (pak) and three stub-not-impl rows
...
```
<sha> docs(formats): register the container-family docs in the inventory
<sha> docs(formats): add IoStore .uptnl stub
<sha> docs(formats): add IoStore .ucas stub
<sha> docs(formats): add IoStore .utoc stub
<sha> docs(formats): add .pak v1-v11 reference
```

If the log is out of order or missing a commit, fix before pushing.

- [ ] **Step 11: Push the branch**

Run: `git push -u origin docs/ue-format-docs-containers`
...
Dispatch in a SINGLE message with multiple Agent tool calls (per project
memory `feedback_parallel_full_review_panel.md` +
`feedback_always_run_review_panel.md`):

- code-reviewer (general quality + spec adherence + factual accuracy)
- code-architect (oracle citations sound, no fabricated wire details, IoStore stub depth appropriate)
- code-simplifier (any sections over-explained? any prose that should be tightened?)

Address issues, re-run on the fix commit, repeat until every reviewer says
APPROVED (per `feedback_review_until_convergence.md`).

---

## Done criteria

- 5 commits on `docs/ue-format-docs-containers` (one per doc + inventory).
- `paksmith-doc-lint required-headings docs/formats/` exits 0.
- `paksmith-doc-lint status-enum docs/formats/README.md` exits 0.
- `typos docs/formats/container/` clean.
- `RUSTDOCFLAGS="-D warnings" cargo doc --workspace --no-deps --all-features` clean.
- PR open with `--body-file`-generated body and lowercase verb-first title.
- Reviewer panel converged.
- Four rows present in `docs/formats/README.md` inventory: one
  `complete | complete` (pak) and three `partial | not impl` (IoStore).
